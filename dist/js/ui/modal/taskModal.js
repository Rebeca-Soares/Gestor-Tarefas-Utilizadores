...
import { editTaskModal, editTaskInput, editTaskCategory, saveEditTaskBtn, closeModalBtn, commentModal, comentsContainer, closeCommentModal, addCommentBtn, newCommentInput, addTaskModal, input, deadlineInput, userSelect, editTaskUser, attachmentError, fileInput, fileNameDisplay } from "../dom/taskDom.js";
import { editTaskPriority, editTaskStatus, editTaskDeadline } from "../dom/taskDom.js";
...
function fillUserDropdown(selectElement) {
    if (!selectElement)
        return;
...
export function openAddTaskModal() {
    if (addTaskModal) {
        if (userSelect)
            fillUserDropdown(userSelect);
        addTaskModal.classList.add('show');
        if (input)
            input.focus();
    }
}
export function closeAddTaskModalFunc() {
    if (addTaskModal) {
...
export function showAttachmentModal(taskId) {
    const renderAttachments = () => {
        ...
        attachmentsContainer.querySelectorAll('.btn-del-attach').forEach(btn => {
            btn.onclick = () => {
                const attachId = Number(btn.dataset.id);
    ...
    if (closeAttachmentModal)
        closeAttachmentModal.onclick = () => attachmentModal.classList.remove('show');
    if (addAttachmentBtn) {
...
export function openEditModal(task) {
    ...
    if (editTaskStatus)
        editTaskStatus.value = task.status.toString();
    if (editTaskDeadline) {
...
if (closeModalBtn)
    closeModalBtn.onclick = closeEditModalFunc;
if (saveEditTaskBtn) {
    saveEditTaskBtn.onclick = () => {
        if (taskBeingEdited) {
            ...
            if (editTaskStatus)
                taskBeingEdited.status = Number(editTaskStatus.value);
...
export function showCommentModal(taskId) {
    const renderList = () => {
    ...
    if (closeCommentModal)
        closeCommentModal.onclick = () => commentModal.classList.remove('show');
    if (addCommentBtn) {
